...
PubSubClient::PubSubClient(Client& client, void (*callback)()) {
  this->_state = MQTT_DISCONNECTED;
  setClient(client);
  this->stream = NULL;
  this->bufferSize = 0;
...
PubSubClient::~PubSubClient() { free(this->buffer); }
...
boolean PubSubClient::connect(const char* id, const char* user,
                              ...
                              uint8_t willQos, boolean willRetain,
                              const char* willMessage) {
  return connect(id, user, pass, willTopic, willQos, willRetain, willMessage,
                 1);
}
...
uint32_t PubSubClient::readPacket(uint8_t* lengthLength) {
  ...
  *lengthLength = len - 1;
  ...
  if (isPublish) {
    ...
    if (!readByte(this->buffer, &len)) return 0;
    if (!readByte(this->buffer, &len)) return 0;
    skip = (this->buffer[*lengthLength + 1] << 8) +
           this->buffer[*lengthLength + 2];
    start = 2;
        ...
        if (type == MQTTPUBLISH) {
          if (callback) {
            ...
            this->buffer[llen + 2 + tl] =
                0; /* end the topic as a 'C' string with \x00 */
...
boolean PubSubClient::publish(const char* topic, const char* payload) {
  return publish(topic, (const uint8_t*)payload,
                 payload ? strnlen(payload, this->bufferSize) : 0, false);
}

boolean PubSubClient::publish(const char* topic, const char* payload,
                              boolean retained) {
  return publish(topic, (const uint8_t*)payload,
                 payload ? strnlen(payload, this->bufferSize) : 0, retained);
}
...
boolean PubSubClient::publish(const char* topic, const uint8_t* payload,
  ...
  if (connected()) {
    if (this->bufferSize <
        MQTT_MAX_HEADER_SIZE + 2 + strnlen(topic, this->bufferSize) + plength) {
      // Too long
      return false;
    }
    // Leave room in the buffer for header and variable length field
...
boolean PubSubClient::publish_P(const char* topic, const char* payload,
                                boolean retained) {
  return publish_P(topic, (const uint8_t*)payload,
                   payload ? strnlen(payload, this->bufferSize) : 0, retained);
}
...
int PubSubClient::endPublish() { return 1; }
...
size_t PubSubClient::buildHeader(uint8_t header, uint8_t* buf,
                                 uint16_t length) {
  uint8_t lenBuf[4];
  uint8_t llen = 0;
  uint8_t digit;
...
boolean PubSubClient::write(uint8_t header, uint8_t* buf, uint16_t length) {
  ...
  while ((bytesRemaining > 0) && result) {
    bytesToWrite = (bytesRemaining > MQTT_MAX_TRANSFER_SIZE)
...
uint16_t PubSubClient::getBufferSize() { return this->bufferSize; }
...
void PubSubClient::setIdle(void (*callback)()) { idle = callback; }
